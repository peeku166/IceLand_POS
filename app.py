...
@admin_required
def daily_report():
    # Daily report for today (admin-only)
    user = get_current_user()
    today = date.today()
    start = datetime(today.year, today.month, today.day)
    end = datetime(today.year, today.month, today.day, 23, 59, 59)

    bills = Bill.query.filter(Bill.created_at >= start, Bill.created_at <= end).order_by(Bill.created_at).all()

    # Split by status
    active_bills = [b for b in bills if b.status == 'ACTIVE']
    refunded_bills = [b for b in bills if b.status == 'REFUNDED']
    cancelled_bills = [b for b in bills if b.status == 'CANCELLED']

    gross_sales = sum(b.total_amount for b in active_bills)
    refund_total = sum(b.total_amount for b in refunded_bills)
    cancel_total = sum(b.total_amount for b in cancelled_bills)

    # Net revenue = active - refunds (cancelled ignored for money if nothing collected)
    net_revenue = gross_sales - refund_total

    # Aggregate best-seller items from ACTIVE bills only (using non-refunded quantities)
    item_stats = {}
    for b in active_bills:
        for bi in b.items:
            effective_qty = bi.quantity - (bi.refunded_qty or 0)
            if effective_qty <= 0:
                continue
            key = f"{bi.item.product_code} - {bi.item.name}"
            if key not in item_stats:
                item_stats[key] = {'qty': 0, 'revenue': 0.0}
            item_stats[key]['qty'] += effective_qty
            item_stats[key]['revenue'] += effective_qty * bi.item.price

    best_sellers = sorted(item_stats.items(), key=lambda x: x[1]['qty'], reverse=True)
...
def admin_items():
    ...
    return render_template('admin_items.html', user=user, items=items)
